...
4. **Success / Failure** — the calculation must finish within a 10-second limit. Crucially, the decision of "did this take too long?" is delegated to the Brain: the app reports only the elapsed seconds to the model (the limit itself lives in the navigation system prompt), and the model navigates to **Failure** or **Success** accordingly.
...
- `CalculationDurationUpdated(durationSeconds)` — periodic timer tick that reports only the elapsed seconds; the time limit lives solely in the system prompt. Marked `isDroppableWhenBusy = true`, so ticks are skipped while the Brain is busy.
